...
def gtfs_routes(gtfs, output_f):
	...
	stoptimes_c = csv.reader(io.TextIOWrapper(gtfs.open(stop_times_file, 'r')))
	header =next( stoptimes_c)
	...
	shapes_c = csv.reader(io.TextIOWrapper(gtfs.open(shapes_file, 'r')))

	header = next(shapes_c)
	...
	for shape_id in shapes.keys():
		shape_keys = list(shapes[shape_id].keys())
		shape_keys.sort()
		shape = []
	...
	trips_c = csv.reader(io.TextIOWrapper(gtfs.open(trips_file, 'r')))
	header = next(trips_c)
	...
	# shape
	for route_id, candidate_shapes in trips_ref.items():
		popular_shape, popular_shape_refs = None, 0
		for shape_id, refs in candidate_shapes.items():
	...
	routes_c = csv.reader(io.TextIOWrapper(gtfs.open(routes_file, 'r')))
	header = next(routes_c)
	route_id_col = header.index('route_id')
